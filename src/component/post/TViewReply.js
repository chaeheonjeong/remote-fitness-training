...
const TViewReply = ({ write, setWrite, writer }) => {
  ...
  const fetchR_Reply = async (rid) => {
    try {
      const res = await axios.get(
        `${BASE_API_URI}/getTR_Reply/${id}/${rid}`,
        {
          headers: { Authorization: `Bearer ${user.token}` },
        }
      );
  ...
  const rhandleSubmit = async (e) => {
    ...
    const handleRDelete = async (rrid) => {
        ...
        if(confirmRDelete) {
        try {
            const response = await axios.delete(`${BASE_API_URI}/postTr_reply/${id}/${selectedRId}/${rrid}`, {
            headers: { Authorization: `Bearer ${user.token}` },
          }
        );
        console.log(response.data);
  ...
  const modifyRHandleSubmit = async (e, selectedRId, rrid) => {
    ...
    try {
      const response = await axios.post(
        `${BASE_API_URI}/viewTReplyRModify`,
        {
          postRId: id,
