...
function MainOpenStudy() {
  ...
  const searchResult = () => {
    ...
    axios
      .get(
        `${BASE_API_URI}/searchOpenStudy?selected=${selected}&value=${encodeURIComponent(
          searchInput
        )}&page=${page}&limit=4`
      )
      .then((response) => {
        ...
        try {
          ...
          setSearchResults((prevSearchOpenStudies) => [
            ...prevSearchOpenStudies,
            ...newSearchOpenStudies,
          ]);
  ...
  const moreOpenStudies = () => {
    ...
    axios
      .get(`${BASE_API_URI}/openStudies?page=${page}&limit=6`)
      .then((response) => {
        ...
        setOpenStudies((prevOpenStudies) => [
          ...prevOpenStudies,
          ...newOpenStudies,
        ]);
        console.log(
          "Number of loaded studies: " +
            (prevOpenStudies.length + newOpenStudies.length)
        );
      ...
      <div className={mainStyles.body}>
        <div className={mainStyles.menu}>
          <div className={mainStyles.select}>
            <Link to="/">
              <button className={mainStyles.openStudy}>오픈스터디</button>
            </Link>
            <Link to="/study">
              <button className={mainStyles.study}>스터디</button>
            </Link>
            <Link to="/question">
              <button className={mainStyles.question}>질문</button>
            </Link>
          </div>
